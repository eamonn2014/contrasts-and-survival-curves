...
kmplot <- function(km, mark=3, simple=FALSE,
  ...
  if(ng==1 & (group.names[1]=='group.names') ){ group.names <- 'Number at risk' ; label.n.at.risk = FALSE }
  ...
  if(!simple){
    ...
    if(label.n.at.risk) mtext( side=1, text='Number at risk', at=group.name.pos, line=1.5, adj=1, col=1, las=1, cex=cex.axis )
  } ## End of if(!simple)	
